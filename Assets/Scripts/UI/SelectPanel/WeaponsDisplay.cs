...
public class WeaponsDisplay : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Btn.onClick.AddListener(delegate {
            if (isUpdate)
                return;
            // 通知宿主选择该武器
            mSelectWeaponsUI.SetWeaponsSelected(type, shape);
        });
    ...
    {
        return GameManager.Instance.GetGameObjectResource(FactoryType.UIFactory, "SelectPanel/WeaponsDisplay").GetComponent<WeaponsDisplay>();
    }
    ...
    {
        GameManager.Instance.PushGameObjectToFactory(FactoryType.UIFactory, "SelectPanel/WeaponsDisplay", gameObject);
    }
}
