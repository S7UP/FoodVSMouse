...
/// 主线任务中的关卡选项条
/// </summary>
public class Item_WarriorChallengePanel : MonoBehaviour
{
    private Button Btn;
    ...
    public static Item_WarriorChallengePanel GetInstance(UnityAction call, int rank, bool isLock, string name, string condition)
    {
        Item_WarriorChallengePanel item = GameManager.Instance.GetGameObjectResource(FactoryType.UIFactory, "WarriorChallengePanel/Item").GetComponent<Item_WarriorChallengePanel>();
        item.Initial();
        ...
        else
        {
            ...
            item.Img_Item.sprite = GameManager.Instance.GetSprite("UI/WarriorChallengePanel/Item_WarriorChallenge");
            if (rank > -1)
            {
                item.Img_Medal.gameObject.SetActive(true);
    ...
    {
        GameManager.Instance.PushGameObjectToFactory(FactoryType.UIFactory, "WarriorChallengePanel/Item", gameObject);
    }
}
