...
public class BaseCardBuilder : MonoBehaviour, IBaseCardBuilder, IGameControllerMember
{
    ...
    {
        // 系统当前卡槽尝试切换成其他卡槽时事件
        BaseCardBuilder lastBuilder = mGameController.mCardController.GetSelectCardBuilder();
        if (lastBuilder!=null && lastBuilder.OnTrySelectOther != null)
        {
            lastBuilder.OnTrySelectOther(mGameController.mCardController.GetSelectCardBuilder(), this);
        }
        ...
        if (CanSelect())
        {
            mGameController.mCardController.SelectCard(arrayIndex); // 通知建造控制器选取自身
        }
        else
        {
            Debug.Log("不可选取，原因是：");
