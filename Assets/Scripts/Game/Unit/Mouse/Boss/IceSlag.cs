...
public class IceSlag : BossUnit
{
    ...
    private void CreateLightBullet(float minLeftX)
    {
        ...
        if(targetUnit == null || !targetUnit.IsAlive())
        {
            ...
            b.taskController.AddTask(TaskManager.GetParabolaTask(b, 60, dist / 2, transform.position, targetPos, true));
        }
        else
        {
            float dist = (transform.position - targetUnit.transform.position).magnitude;
            b.taskController.AddTask(TaskManager.GetParabolaTask(b, 60, dist / 2, transform.position, targetUnit, true));
        }
        GameController.Instance.AddBullet(b);
    }
