...
/// 芝士城堡
/// </summary>
public class Map_CheeseCastle : ChapterMap
{
    ...
    public void Awake()
    {
        if(SpriteArray == null)
        {
            ...
            for (int i = 0; i < 3; i++)
                SpriteArray[i] = GameManager.Instance.GetSprite("Chapter/1/4/" + i);
        }
    ...
    private void ChangeAction(string key, List<float> oldArray, List<float> newArray)
    {
        ...
        if(oldValue != newValue)
        {
            ...
            renderer0.sprite = GameManager.Instance.GetSprite("Chapter/1/4/" + newValue);
            AddWater(newValue);
        }
    }
