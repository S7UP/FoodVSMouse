...
/// 香料飞船
/// </summary>
public class Map_SpiceSpaceship2 : ChapterMap
{
    /// <summary>
    /// 创建格子
    /// </summary>
    ...
    public override void OtherProcessing()
    {
        ...
        // 添加云层(345)
        for (int i = 2; i <= 4; i++)
            Item_Cloud.GetCloudGroup(2, new Vector2(MapManager.GetColumnX(7f), MapManager.GetRowY(i)), 1);
        // 添加云层(1267)
        for (int i = 0; i <= 1; i++)
        {
            Item_Cloud.GetCloudGroup(2, new Vector2(MapManager.GetColumnX(4f), MapManager.GetRowY(i)), 9);
            Item_Cloud.GetCloudGroup(2, new Vector2(MapManager.GetColumnX(4f), MapManager.GetRowY(5 + i)), 9);
        }
