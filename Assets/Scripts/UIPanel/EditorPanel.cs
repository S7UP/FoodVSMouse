namespace EditorPanel
{
    ...
    public class EditorPanel : BasePanel
    {
        ...
        private Button Btn_CopyRound;
        private Button Btn_InsertAndReturn;
        private Button Btn_Save; // 保存
        private Button Btn_SaveAll; // 保存全部
        private Button Btn_Open; // 打开关卡目录
        private InputField Inf_FileName; // 当前关卡文件名
        ...
        private GameObject Emp_IsSkipWhenBossDie;
        private Toggle Tog_IsSkipWhenBossDie; // 是否跳过这一阶段刷怪当BOSS死亡时
        ...
        // 当前待编辑的关卡信息
        private BaseStage.StageInfo currentStageInfo { get { return GlobalData.Manager.Instance.mEditStage.GetStageInfo(); } }
        // 当前轮级目录索引栈
        ...
        protected override void Awake()
        {
            ...
            Btn_CopyRound = transform.Find("MouseEditorUI").Find("Emp_RoundNavigation").Find("Btn_CopyRound").GetComponent<Button>();
            Btn_CopyRound.onClick.AddListener(delegate { GlobalData.Manager.Instance.mEditStage.CopyRoundInfo(GetCurrentRoundInfo()); });
            Btn_InsertAndReturn = transform.Find("MouseEditorUI").Find("Emp_RoundNavigation").Find("Btn_InsertAndReturn").GetComponent<Button>();
            Btn_InsertAndReturn.onClick.AddListener(delegate {
                if (GlobalData.Manager.Instance.mEditStage.GetCopiedRoundInfo() == null)
                    return;
                if (currentRoundIndexStack.Count == 1)
                {
                    GetCurrentStageInfo().roundInfoList.Insert(currentRoundIndexStack.Peek(), GlobalData.Manager.Instance.mEditStage.GetCopiedRoundInfo().DeepCopy());
                }
                else
                {
                    RoundInfo r = roundInfoStack.Pop();
                    roundInfoStack.Peek().roundInfoList.Insert(currentRoundIndexStack.Peek(), GlobalData.Manager.Instance.mEditStage.GetCopiedRoundInfo().DeepCopy());
                    roundInfoStack.Push(r);
                }
                ReturnLast();
            });
            Btn_Save = transform.Find("Btn_Save").GetComponent<Button>();
            Btn_Save.onClick.AddListener(Save);
            Btn_SaveAll = transform.Find("Btn_SaveAll").GetComponent<Button>();
            Btn_SaveAll.onClick.AddListener(SaveAll);

            Btn_Open = transform.Find("Btn_Open").GetComponent<Button>();
            Btn_Open.onClick.AddListener(delegate {
                ...
                if (!edit.IsUseCustStageList())
                    Application.OpenURL(Application.streamingAssetsPath + "/Json/Stage");
                else
                    Application.OpenURL(Application.persistentDataPath + "/CustomStage");
            });
            Inf_FileName = transform.Find("StageEditorUI").Find("Img_RoundInfo").Find("Emp_FileName").Find("InputField").GetComponent<InputField>();
            Inf_FileName.onEndEdit.AddListener(delegate {
                if (!isUIUpdateState)
                {
                    currentStageInfo.fileName = Inf_FileName.text;
                }
            });
            ...
            Emp_IsSkipWhenBossDie = transform.Find("MouseEditorUI").Find("Img_RoundInfo").Find("Emp_IsSkipWhenBossDie").gameObject;
            Tog_IsSkipWhenBossDie = Emp_IsSkipWhenBossDie.transform.Find("Toggle").GetComponent<Toggle>();
            Tog_IsSkipWhenBossDie.onValueChanged.AddListener(delegate { OnIsSkipWhenBossDieToggleClick(); });
        ...
        public void LoadStage(BaseStage.StageInfo info)
        {
            ...
            Inf_FileName.text = currentStageInfo.fileName;
            //Inf_FileName.interactable = edit.IsUseCustStageList();
            Inf_FileName.interactable = false;
        ...
        private void UpdateMouseEditorUI()
        {
            ...
            if (!isUpdateBossToggle)
            {
                ...
                Emp_IsSkipWhenBossDie.gameObject.SetActive(currentRoundInfo.isBossRound);
            }
            ...
            Tog_IsSkipWhenBossDie.isOn = currentRoundInfo.isSkipWhenBossDie;
        ...
        private void Save()
        {
            ...
            {
                edit.SaveCustStage(edit.GetStageInfo());
            }
        ...
        private void SaveAll()
        {
            ...
            {
                BaseStage.Save(currentStageInfo);
                Tex_FloatFont.text = "保存全部不支持该编辑模式！仅保存当前关卡！";
            }
            else
            {
                foreach (var info in edit.GetCustStageList())
                {
                    edit.SaveCustStage(info);
                }
                Tex_FloatFont.text = "所有实验室关卡保存成功！";
            }
            AlphaDecTweener.Restart();
        }
        ...
        private void OnYesDeleteBtnClick()
        {
            ...
            if (!edit.IsUseCustStageList())
            {
                ...
                // 重载
                {
                    BaseStage.StageInfo info = GlobalData.Manager.Instance.mEditStage.GetStageInfo();
                    GameManager.Instance.attributeManager.ReloadStageInfoListFromScene(info.chapterIndex, info.sceneIndex);
                }
            }
            else
            {
                edit.DeleteCustStage(edit.GetCustStageList()[edit.GetSelectedCustStageIndex()]);
            }

            // 刷新关卡下拉列表
            mImg_StagePath.UpdateSelection();
            // 隐藏
            Img_Confirm.SetActive(false);
            // 给出提示
        ...
        private void OnIsBossRoundToggleClick()
        {
            ...
            if (isBoss)
            {
                ...
                // 显示是否跳过
                Emp_IsSkipWhenBossDie.gameObject.SetActive(true);
            }
            else
            {
                // 当选择为非BOSS模式时
        ...
        private void OnIsSkipWhenBossDieToggleClick()
        {
            if (isUIUpdateState)
                return;
            GetCurrentRoundInfo().isSkipWhenBossDie = Tog_IsSkipWhenBossDie.isOn;
            UpdateUI();
        }
        #endregion
