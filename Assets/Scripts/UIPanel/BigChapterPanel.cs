    ...
    public class BigChapterPanel : BasePanel
    {
        ...
        private void UpdateItemListAndItsDisplay()
        {
            ...
            foreach (var static_info in currentChapter.stageList)
            {
                ...
                Item item = Item.GetInstance(local_info.rank, !ConfigManager.IsDeveloperMode() && !local_info.isUnlocked, static_info.id + " " + info.name, info.difficulty, static_info.GetUnLockCondition(), local_info.diffRate, local_info.cardLevelArray, local_info.cardCountArray);
                itemList.Add(item);
